...
app.config['UPLOAD_GALLERY'] = 'static/uploads/gallery'
...
@app.route('/')
def index():
    path = app.config['UPLOAD_GALLERY']
    ...
    # Pass images into template
    return render_template('index.html', images=gallery_files)
...
def admin_dashboard():
    ...
    gallery_files = os.listdir(app.config['UPLOAD_GALLERY'])

    return render_template('admin_dashboard.html', sermons=sermon_files, posters=poster_files, leaders=leaders, messages=messages, gallery=gallery_files)
...
def gallery():
    ...
    return render_template('gallery.html', images=gallery_files)
...
def upload_gallery():
    ...
    flash(f"Gallery image '{filename}' uploaded successfully!")
    return redirect(url_for('admin_dashboard'))

@app.route('/delete_gallery/<filename>', methods=['POST'])
def delete_gallery(filename):
    if not session.get('admin_logged_in'):
        return redirect(url_for('admin_login'))

    path = os.path.join(app.config['UPLOAD_GALLERY'], filename)
    if os.path.exists(path):
        os.remove(path)
        flash(f"Gallery image '{filename}' deleted.")
    return redirect(url_for('admin_dashboard'))
...
def manage_sermons():
    ...
    path = app.config['UPLOAD_SERMONS']
    os.makedirs(path, exist_ok=True)
    sermons = os.listdir(path)
    return render_template('manage_sermons.html', sermons=sermons)
...
def manage_events():
    ...
    current_events = Event.query.filter(Event.end_date >= today).all()
    archived_events = Event.query.filter(Event.end_date < today).all()
    posters = os.listdir(app.config['UPLOAD_POSTERS'])
...
def manage_gallery():
    ...
    path = app.config['UPLOAD_GALLERY']
    os.makedirs(path, exist_ok=True)
    gallery_files = os.listdir(path)
    return render_template('manage_gallery.html', gallery=gallery_files)
